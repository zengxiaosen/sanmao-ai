...
1. **本机代理可能会污染普通 SSH 连接**
   ...
   - 如果后来确认远端可以**直接访问**目标国内源（例如 `www.modelscope.cn:443` 可达），应优先去掉反向代理依赖，避免长时间下载任务因为本地隧道中断而卡死。
...
3. **旧 `qwen3-8b-awq` 目前可以停用为 coding 主力，但还不能直接删**
   ...
   - 当前已完成的进一步状态：
     - `download_llm_model.sh` 不再允许把 `qwen3-8b-awq` 当作活跃下载目标
     - `verify_all.sh` / `start_server_workflow.sh` 默认不再自动跑旧 8B 检查
     - 远端 `qwen3-8b-awq` 目录已删除，只在需要兼容时才手动启用旧链路

4. **Blackwell + vLLM + FlashInfer 需要额外兼容处理**
   - 当前机器 `torch.cuda.get_device_capability(0)` 返回 `(12, 0)`，属于 Blackwell。
   - 默认启动参数下，vLLM/FlashInfer 可能在采样/JIT 初始化阶段报：
   ...
   - 当前仓库的应对经验：
     ...
     - 如果仍失败，再进一步在服务启动参数里强制：
       - `--attention-backend FLASH_ATTN`
       - `--attention-config.use_trtllm_attention 0`
     - 如果切到 `FLASH_ATTN` 后再报 `kv_cache_dtype not supported`，说明这条路径暂时不能继续使用 `fp8` KV cache，应先回退到 `bfloat16`，优先把服务跑通。
   - 当前这台机器的保守可用启动组合已经验证可启动：
     - `attention-backend=FLASH_ATTN`
     - `use_trtllm_attention=0`
     - `kv-cache-dtype=bfloat16`
   - 当前实测成功标准：
     - `/health` 返回 200
     - `/v1/models` 返回 `qwen3-coder-30b-a3b-instruct-fp8`
     - `/v1/chat/completions` 返回 200
     - 服务常驻时显存占用约 89.9 GiB、剩余约 7.3 GiB
   - 先用这组保守兼容参数把服务跑通，再考虑是否进一步优化采样或 attention backend。
