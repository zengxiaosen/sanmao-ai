...
| 脚本 | 用途 |
|---|---|
| `bootstrap_server.sh` | 创建/更新量化 `.venv` |
| `download_llm_model.sh` | 创建/更新 LLM 环境，并下载 Qwen 模型 |
| `open_hf_proxy_tunnel.sh` | 本机到服务器的 Hugging Face 代理隧道 |
| `setup_qwen3_coder_vllm.sh` | 创建/更新 Qwen3-Coder 的 vLLM 环境，并下载 FP8 模型 |
| `start_qwen3_coder_vllm.sh` | 启动 Qwen3-Coder 的 vLLM OpenAI-compatible 服务 |
| `setup_server_all.sh` | 新机器一键完整部署 |
...
| `extract_news_with_llm.py` | 用本地 Qwen 把新闻抽成 JSON 结构化事件 |
...
| `run_all.sh` | run 目录总入口：取数据 -> Qwen 抽取 -> 拼特征 -> 训练 -> 预测 -> 回测 |
...
| `smoke_llm_qwen.py` | 检查 Qwen 是否能加载并输出 JSON |
| `check_qwen3_coder_vllm.sh` | 检查 Qwen3-Coder 的 vLLM OpenAI-compatible 服务 |
| `show_qwen3_coder_endpoint.sh` | 打印 Qwen3-Coder 的 OpenAI-compatible 接入方式 |
| `show_qwen3_coder_clients.sh` | 打印 Aider / OpenAI client / 通用 agent 的接入示例 |
| `show_aider_qwen3_coder.sh` | 打印 Aider 连接 Qwen3-Coder 的示例 |
| `show_openhands_qwen3_coder.sh` | 打印 OpenHands 连接 Qwen3-Coder 的示例 |
| `smoke_qwen3_coder_agent.sh` | 用 Qwen3-Coder vLLM 做最小 coding-agent smoke test |
| `start_server_workflow.sh` | 服务器开机后一键验证当前链路 |
...
4. 不要把下载模型、安装依赖的逻辑写进 `scripts/run/`。
