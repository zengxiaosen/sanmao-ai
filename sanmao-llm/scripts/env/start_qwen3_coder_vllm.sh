...
CUDA_HOME="${CUDA_HOME:-/root/autodl-tmp/vllm-env/lib/python3.12/site-packages/nvidia/cu13}"

cd "$PROJECT_DIR"
mkdir -p "$LOG_DIR" "$VLLM_CACHE_ROOT" "$HF_HOME" "$HUGGINGFACE_HUB_CACHE" "$TRANSFORMERS_CACHE"

export HF_HOME HUGGINGFACE_HUB_CACHE TRANSFORMERS_CACHE VLLM_CACHE_ROOT CUDA_HOME
export PATH="$VLLM_ENV/bin:$PATH"
export VLLM_USE_FLASHINFER_SAMPLER="${VLLM_USE_FLASHINFER_SAMPLER:-0}"
ATTENTION_BACKEND="${ATTENTION_BACKEND:-FLASH_ATTN}"
USE_TRTLLM_ATTENTION="${USE_TRTLLM_ATTENTION:-0}"
KV_CACHE_DTYPE_FLASH_ATTN="${KV_CACHE_DTYPE_FLASH_ATTN:-bfloat16}"
...
nohup "$VLLM_ENV/bin/vllm" serve "$MODEL_PATH" \
  ...
  --attention-config.use_trtllm_attention "$USE_TRTLLM_ATTENTION" \
