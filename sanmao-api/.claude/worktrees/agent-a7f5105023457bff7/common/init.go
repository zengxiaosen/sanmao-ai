...
const sessionSecretFileName = ".session_secret"

func loadOrCreateSessionSecret(secretPath string) (string, error) {
	if secretPath == "" {
...
func InitEnv() {
	...
	if os.Getenv("SESSION_SECRET") != "" {
		...
		secretPath := filepath.Join(filepath.Dir(SQLitePath), sessionSecretFileName)
		if secret, err := loadOrCreateSessionSecret(secretPath); err != nil {
			log.Printf("WARNING: failed to persist session secret at %s: %v", secretPath, err)
		} else {
