...
func TestMaybeInvalidateChannelAffinityOnFailure_RemovesCacheForDoRequestFailed(t *testing.T) {
	gin.SetMode(gin.TestMode)

	cache := getChannelAffinityCache()
	cacheKey := "new-api:channel_affinity:v1:test-rule\ndefault\nfp-123"
	...
	doRequestErr := types.NewOpenAIError(
		errors.New("upstream error: do request failed"),
		types.ErrorCodeDoRequestFailed,
		http.StatusInternalServerError,
	)

	invalidated := MaybeInvalidateChannelAffinityOnFailure(ctx, doRequestErr)
...
func TestMaybeInvalidateChannelAffinityOnFailure_KeepsCacheForNonTransportFailure(t *testing.T) {
	gin.SetMode(gin.TestMode)

	cache := getChannelAffinityCache()
	cacheKey := "new-api:channel_affinity:v1:test-rule\ndefault\nfp-456"
	require.NoError(t, cache.SetWithTTL(cacheKey, 43, time.Minute))
	t.Cleanup(func() {
		_, _ = cache.DeleteMany([]string{cacheKey})
	})
	...
	badResponseErr := types.NewOpenAIError(
		errors.New("bad response"),
		types.ErrorCodeBadResponse,
		http.StatusInternalServerError,
	)

	invalidated := MaybeInvalidateChannelAffinityOnFailure(ctx, badResponseErr)
	require.False(t, invalidated)

	channelID, found, err := cache.Get(cacheKey)
	require.NoError(t, err)
	require.True(t, found)
	require.Equal(t, 43, channelID)
}
