...
const getSanmaoRoutingBadge = (record, t) => {
  if (!record || record.children !== undefined) return null;
  if (record.name === 'vision-claude') {
    return (
  ...
  if (record.name === 'andya-gemini' || record.type === 24) {
    return (
      <Tag color='amber' type='light' size='small' shape='circle'>
        {t('已停用')}
      </Tag>
    );
  }
  if (record.name === 'vision-codex') {
    return (
...
export const getChannelsColumns = ({
  ...
  return [
    ...
    },
    {
      ...
      render: (text, record, index) => {
        ...
        if (!passThroughEnabled && !showUpstreamUpdateTag) {
          const routingBadge = getSanmaoRoutingBadge(record, t);
          if (!routingBadge) {
            return nameNode;
          }
          return (
        ...
        return (
          <Space spacing={6} align='center'>
            {nameNode}
            {getSanmaoRoutingBadge(record, t)}
            {passThroughEnabled && (
              <Tooltip
                content={t(
    ...
    },
    {
      ...
      dataIndex: 'status',
      render: (text, record, index) => {
        if (record?.name === 'andya-gemini' && text === 2) {
          return (
            <Tooltip content={t('当前默认业务路线不使用 Gemini，该渠道已从活跃路由中移除。')}>
