...
const CARD_STYLES = {
  container:
    'w-12 h-12 rounded-2xl flex items-center justify-center relative shadow-md border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.92),rgba(241,245,249,0.90))]',
      ...
      <div className='grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-4'>
        {paginatedModels.map((model, index) => {
          ...
          return (
            <Card
              key={modelKey || index}
              className={`!rounded-2xl transition-all duration-200 hover:shadow-xl border cursor-pointer overflow-hidden ${isSelected ? CARD_STYLES.selected : CARD_STYLES.default}`}
                  ...
                  {showRatio && (
                    <div className='pt-3'>
                      <div className='flex items-center space-x-1 mb-2'>
                        ...
                        <Tooltip
                          content={t('倍率用于把不同模型统一换算成本站扣费，输入按模型倍率，输出按模型倍率和输出倍率计算')}
                        >
                          <IconHelpCircle
