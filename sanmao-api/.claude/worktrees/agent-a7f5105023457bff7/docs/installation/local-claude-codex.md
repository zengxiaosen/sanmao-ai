...
For a better day-to-day UX on any machine, use the unified launcher instead of managing environment variables by hand:

```bash
chmod +x scripts/claude-sanmao.sh
bash scripts/claude-sanmao.sh setup
bash scripts/claude-sanmao.sh pick
```
...
```bash
# One-time setup: store the sanmao token locally for this wrapper
bash scripts/claude-sanmao.sh setup

# Show the current sanmao-backed model list
bash scripts/claude-sanmao.sh models

# Interactively choose a model, then launch Claude
bash scripts/claude-sanmao.sh pick

# Launch directly with an explicit model
bash scripts/claude-sanmao.sh --model glm-5.2

# Clear the remembered default model
bash scripts/claude-sanmao.sh clear-default
```
...
```bash
bash scripts/claude-sanmao.sh --print-env
```
