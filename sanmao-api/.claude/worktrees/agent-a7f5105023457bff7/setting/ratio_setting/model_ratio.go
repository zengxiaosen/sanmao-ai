...
var claudeModelRatioFallbacks = []string{
	"claude-opus-4-6",
...
var defaultModelRatio = map[string]float64{
	...
	"glm-5.2":                                   0.4110,
	"glm-5v-turbo":                              0.3425,
	"glm-4.1v-thinking-flashx":                  0,
	"qwen-turbo":                                0.8572, // ￥0.012 / 1k tokens
	"qwen-plus":                                 10,     // ￥0.14 / 1k tokens
	"qwen3.7-plus":                              0.14285714285714285,
	"qwen3.7-plus-2026-05-26":                   0.14285714285714285,
	"qwen3.7-max":                               0.8571428571428571,
	"qwen3.7-max-2026-06-08":                    0.8571428571428571,
...
var defaultCompletionRatio = map[string]float64{
	...
	"gpt-image-1":              8,
	"qwen3.7-plus":             4,
	"qwen3.7-plus-2026-05-26":  4,
	"qwen3.7-max":              3,
	"qwen3.7-max-2026-06-08":   3,
	"deepseek-v4-pro":          2,
	"deepseek-v4-flash":        2,
	"glm-5":                    5.5,
	"glm-5-turbo":              4.4,
...
func GetModelRatio(name string) (float64, bool, string) {
	...
	if !ok {
		if fallbackName, fallbackRatio, fallbackOK := getClaudeModelRatioFallback(name); fallbackOK {
			return fallbackRatio, true, fallbackName
		}
		if strings.HasSuffix(name, CompactModelSuffix) {
...
func GetCompletionRatio(name string) float64 {
	...
	if fallbackName, _, fallbackOK := getClaudeModelRatioFallback(name); fallbackOK {
		if ratio, ok := completionRatioMap.Get(fallbackName); ok {
			return ratio
		}
	}
	return hardCodedRatio
...
func ContainsExactModelRatio(name string) bool {
	name = FormatMatchingModelName(name)
	_, ok := modelRatioMap.Get(name)
	return ok
}

func ContainsExactModelPrice(name string) bool {
	name = FormatMatchingModelName(name)
	_, ok := modelPriceMap.Get(name)
	return ok
}

func ContainsExactCompletionRatio(name string) bool {
	name = FormatMatchingModelName(name)
	_, ok := completionRatioMap.Get(name)
	return ok
}

func getClaudeModelRatioFallback(name string) (string, float64, bool) {
	for _, fallbackName := range claudeModelRatioFallbackCandidates(name) {
		if ratio, ok := modelRatioMap.Get(fallbackName); ok {
			return fallbackName, ratio, true
		}
	}
	return "", 0, false
}

func claudeModelRatioFallbackCandidates(name string) []string {
	switch {
	...
	return claudeModelRatioFallbacks
}
