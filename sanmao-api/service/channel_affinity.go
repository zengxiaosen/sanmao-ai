...
func clearChannelAffinityCacheKey(cacheKey string) bool {
...
// MaybeInvalidateChannelAffinityOnFailure clears a sticky channel-affinity entry
// when the upstream transport itself failed. This prevents clients such as
// Claude CLI from being pinned to a broken channel for the full affinity TTL.
func MaybeInvalidateChannelAffinityOnFailure(c *gin.Context, err *types.NewAPIError) bool {
	if c == nil || err == nil {
		return false
	}
	...
	return clearChannelAffinityCacheKey(cacheKey)
}
