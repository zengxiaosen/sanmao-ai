...
var fetchGeminiModels = gemini.FetchGeminiModels
...
func validateChannel(channel *model.Channel, isAdd bool) error {
	...
	// Codex credential validation (supports OAuth JSON and plain sk- API keys)
	if channel.Type == constant.ChannelTypeCodex {
		trimmedKey := strings.TrimSpace(channel.Key)
		if isAdd || trimmedKey != "" {
			mode, err := codex.DetectCredentialMode(trimmedKey)
			if err != nil {
				return err
			}
			if mode == codex.CredentialModeOAuth {
...
func FetchModels(c *gin.Context) {
	var req struct {
		...
		HeaderOverride json.RawMessage `json:"header_override"`
	}
	...
	channel := &model.Channel{
		Type: req.Type,
		Key:  strings.Split(trimmedKey, "\n")[0],
	}
	if req.BaseURL != "" {
		channel.BaseURL = common.GetPointer(strings.TrimSpace(req.BaseURL))
	}
	if strings.TrimSpace(req.Proxy) != "" || len(req.HeaderOverride) > 0 {
		setting := dto.ChannelSettings{Proxy: strings.TrimSpace(req.Proxy)}
		channel.SetSetting(setting)
		if len(req.HeaderOverride) > 0 {
			override := strings.TrimSpace(string(req.HeaderOverride))
			channel.HeaderOverride = common.GetPointer(override)
		}
	}
