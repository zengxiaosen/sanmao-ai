...
export const useDashboardData = (userState, userDispatch, statusState) => {
  ...
  const loadBusinessStats = useCallback(async () => {
    ...
    const url = isAdminUser
      ? `/api/log/stat?${params.toString()}`
      : `/api/log/self/stat?${params.toString()}`;
    const res = await API.get(url);
    const { success, data, message } = res.data;
    if (!success) {
      showError(message);
      return { quota: 0, count: 0, tokens: 0 };
    }
    const stats = {
      quota: Number(data?.quota || 0),
      count: Number(data?.count || data?.rpm || 0),
      tokens: Number(data?.tokens || data?.tpm || 0),
      upstreamCostQuota: Number(data?.upstream_cost_quota || 0),
      usageCostQuota: Number(data?.usage_cost_quota || 0),
      fixedCostQuota: Number(data?.fixed_cost_quota || 0),
      netProfitQuota: Number(data?.net_profit_quota || 0),
      profitRate: Number(data?.profit_rate || 0),
      costConfigured: Boolean(data?.cost_configured),
      costConfiguredLogs: Number(data?.cost_configured_logs || 0),
      costUnconfiguredLogs: Number(data?.cost_unconfigured_logs || 0),
    };
