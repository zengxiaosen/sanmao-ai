...
export const useChannelsData = () => {
  ...
  const initialStatusFilter = localStorage.getItem('channel-status-filter');
  ...
  useEffect(() => {
    ...
    if (!initialStatusFilter || initialStatusFilter === 'all') {
      localStorage.setItem('channel-status-filter', 'enabled');
    }
  ...
  const handleChannelSaved = async (channel) => {
    if (!channel?.id || !channel?.models) {
      return;
    }
    ...
    showInfo(
      t('已自动验证渠道 ${name} 的默认模型 ${model}')
        .replace('${name}', channel.name || String(channel.id))
        .replace('${model}', firstModel),
    );
    await testChannel(channel, firstModel);
  };
