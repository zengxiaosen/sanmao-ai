  ...
  Object.entries(config.channels || {}).forEach(([channelId, channelConfig]) => {
    if (channelConfig?.default) {
      rows.push({
        ...
        request_cost: Number(channelConfig.default.request_cost || 0),
      });
    }
    Object.entries(channelConfig?.models || {}).forEach(([modelName, modelConfig]) => {
      rows.push({
        ...
        request_cost: Number(modelConfig.request_cost || 0),
      });
    });
  });
  return rows;
...
function rowsToConfig(rows) {
  ...
  rows.forEach((row) => {
    ...
    const cost = {
      ...
      request_cost: Number(row.request_cost || 0),
    };
...
function mergeConfig(rows, fixedCostRows) {
  ...
  const fixedCosts = fixedCostRows
    ...
    .map((row) => ({
      ...
      end_timestamp: Number(row.end_timestamp || 0),
...
export default function SettingsBusinessCost(props) {
  ...
  const addFixedCostRow = () => {
    setFixedCostRows((prev) => [
      ...prev,
      {
        ...
        currency: 'CNY',
        period: 'month',
        start_timestamp: 1781506800,
        end_timestamp: 1784098800,
        channel_ids: [],
      },
    ]);
  };
  ...
  const addVisionCoderCosts = () => {
    setRows((prev) => {
      ...
      const additions = [];
      Object.entries(VISION_MODEL_POINTS).forEach(([modelName, requestPoints]) => {
        [
          ...
          additions.push({
            ...
            input_per_million: 0,
            output_per_million: 0,
            request_cost: Number((requestPoints * VISION_CREDIT_USD).toFixed(9)),
          });
        });
      });
      if (additions.length === 0) {
  ...
  const onSubmit = async () => {
    ...
    for (const row of rows) {
      ...
      if (Number(row.input_per_million) < 0 || Number(row.output_per_million) < 0 || Number(row.request_cost || 0) < 0) {
    ...
    for (const row of fixedCostRows) {
      ...
      if (
        (Number(row.start_timestamp || 0) > 0 || Number(row.end_timestamp || 0) > 0) &&
        Number(row.end_timestamp || 0) <= Number(row.start_timestamp || 0)
      ) {
        showError(t('固定成本的结束时间必须晚于开始时间'));
        return;
      }
    }
    ...
    },
    {
      ...
      render: (_, record) => (
        <InputNumber
          ...
          step={0.0001}
          precision={9}
          value={record.request_cost}
          suffix='$/次'
          onChange={(value) => updateRow(record.id, { request_cost: Number(value || 0) })}
        />
      ),
    },
    {
  ...
  const fixedCostColumns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <DatePicker
          type='dateTime'
          value={timestampToDate(record.start_timestamp)}
          onChange={(value) => updateFixedCostRow(record.id, { start_timestamp: dateToTimestamp(value) })}
          placeholder={t('不填则按周期长期折算')}
        />
      ),
    },
    {
      ...
      render: (_, record) => (
        <DatePicker
          type='dateTime'
          value={timestampToDate(record.end_timestamp)}
          onChange={(value) => updateFixedCostRow(record.id, { end_timestamp: dateToTimestamp(value) })}
          placeholder={t('不填则按周期长期折算')}
        />
      ),
    },
      ...
      <Form.Section text={t('经营成本配置')}>
        <Banner
          ...
          description={
            <div>
              ...
              <br />
              <Text type='tertiary'>
                {t('按量成本支持美元 / 100万 Tokens，也支持美元 / 每次请求。固定成本适合阿里云 Token Plan、包月套餐、GPU 小时租用、服务器月租等，会按筛选时间自动折算。')}
              </Text>
            </div>
          }
        />
        <Space wrap style={{ margin: '14px 0' }}>
          ...
          <Button onClick={addVisionCoderCosts}>
            {t('按 ¥150/35000 积分/次导入 VisionCoder')}
          </Button>
        </Space>
        ...
        <div style={{ marginTop: 18 }}>
          <Space wrap style={{ marginBottom: 12 }}>
            <Text strong>{t('固定成本：适合阿里云 Token Plan、GPU 小时租用、包月套餐')}</Text>
